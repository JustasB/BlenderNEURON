...
class CUSTOM_OT_cell_group_add(Operator, CUSTOM_OT_cellgroup_operator):
    ...
    bl_description = "Add new cell display group. Each group allows cells within it to have their own granularity and variable recording settings. E.g. to allow customization of cell display detail"
...
class CUSTOM_OT_get_cell_list_from_neuron(Operator, CUSTOM_OT_cellgroup_operator):
    ...
    def update_group_cell_list(self, context, group):
        ...
        group.group_cells.clear()
        
        for i, root in enumerate(roots):
            group_root = group.group_cells.add()
            group_root.id = root.id
            group_root.name = root.name

            # Select the cell if it's not included in other groups
            group_root.selected = root.group_index == -1
    ...
    def execute(self, context):
        ...
        self.update_group_cell_list(context, groups[group_i])
...
class CUSTOM_OT_import_selected_neuron_cells(Operator, CUSTOM_OT_cellgroup_operator):
